...
<aside class="flex w-72 flex-col border-l border-[#E8E2F2] bg-white">
  <div
    class="flex items-center justify-between border-b border-[#E8E2F2] px-4 py-3"
  >
    <div>
  ...
  <div class="space-y-5 overflow-y-auto p-4">
    ...
    {#if !question}
      {#if !currentSection}
        <div
          class="flex h-full flex-col items-center justify-center px-5 text-center py-8"
        >
          <div class="mb-3 rounded-xl bg-[#F3ECFA] p-3 text-[#9B7EBD]">
            <Settings2 size={26} />
          </div>
          <h2 class="text-sm font-semibold text-[#3B1E54]">No Selection</h2>
    ...
    {:else}
      ...
      <section>
        ...
        <div class="space-y-3">
          <div>
            ...
            <Input bind:value={question.label}  onblur={handleQuestionFieldChange}/>
          </div>
          ...
          <div>
            ...
            <Textarea bind:value={question.description} onblur={handleQuestionFieldChange}/>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div
          class="flex items-center justify-between rounded-xl border border-[#E8E2F2] px-3 py-3"
        >
          <div>
            <p class="text-sm font-medium text-[#3B1E54]">Required</p>
            <p class="text-[11px] text-slate-500">Must answer</p>
          </div>
          <Switch bind:checked={question.required} onblur={handleQuestionFieldChange}/>
        </div>
      </section>
      ...
      {#if ["short_text", "long_text", "email"].includes(question.type)}
        <section>
          ...
          <Input bind:value={question.placeholder} onblur={handleQuestionFieldChange}/>
        </section>
      {/if}
          ...
          <div class="space-y-2">
            {#each question.options ?? [] as option, index}
              <div class="flex gap-2">
                <Input bind:value={question.options[index]} onblur={handleQuestionFieldChange} />
                <Button
                  variant="ghost"
                  size="icon"
